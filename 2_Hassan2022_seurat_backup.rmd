...
```{r message=FALSE, warning = FALSE}
data.10x = list(); 
data.10x[[1]] <- Read10X(data.dir = paste0(indir, "CRN00224913/outs/filtered_feature_bc_matrix"));
data.10x[[2]] <- Read10X(data.dir = paste0(indir, "CRN00224914/outs/filtered_feature_bc_matrix"));
data.10x[[3]] <- Read10X(data.dir = paste0(indir, "CRN00224915/outs/filtered_feature_bc_matrix"));
data.10x[[4]] <- Read10X(data.dir = paste0(indir, "CRN00224916/outs/filtered_feature_bc_matrix"));
data.10x[[5]] <- Read10X(data.dir = paste0(indir, "CRN00224917/outs/filtered_feature_bc_matrix"));
data.10x[[6]] <- Read10X(data.dir = paste0(indir, "CRN00224918/outs/filtered_feature_bc_matrix"));
data.10x[[7]] <- Read10X(data.dir = paste0(indir, "CRN00224919/outs/filtered_feature_bc_matrix"));
data.10x[[8]] <- Read10X(data.dir = paste0(indir, "CRN00224920/outs/filtered_feature_bc_matrix"));
data.10x[[9]] <- Read10X(data.dir = paste0(indir, "CRN00224921/outs/filtered_feature_bc_matrix"));
data.10x[[10]] <- Read10X(data.dir = paste0(indir, "CRN00224922/outs/filtered_feature_bc_matrix"));
data.10x[[11]] <- Read10X(data.dir = paste0(indir, "CRN00224923/outs/filtered_feature_bc_matrix"));
data.10x[[12]] <- Read10X(data.dir = paste0(indir, "CRN00224924/outs/filtered_feature_bc_matrix"));
data.10x[[13]] <- Read10X(data.dir = paste0(indir, "CRN00224925/outs/filtered_feature_bc_matrix"));
data.10x[[14]] <- Read10X(data.dir = paste0(indir, "CRN00224926/outs/filtered_feature_bc_matrix"));
```

# Step3: Create Seurat object
```{r message=FALSE, warning = FALSE}
scrna.list = list(); 
scrna.list[[1]] = CreateSeuratObject(counts = data.10x[[1]], min.cells=3, project=samples[1]);
scrna.list[[1]][["DataSet"]] = samples[1];
scrna.list[[2]] = CreateSeuratObject(counts = data.10x[[2]], min.cells=3, project=samples[2]);
scrna.list[[2]][["DataSet"]] = samples[2];
scrna.list[[3]] = CreateSeuratObject(counts = data.10x[[3]], min.cells=3, project=samples[3]);
scrna.list[[3]][["DataSet"]] = samples[3];
scrna.list[[4]] = CreateSeuratObject(counts = data.10x[[4]], min.cells=3, project=samples[4]);
scrna.list[[4]][["DataSet"]] = samples[4];
scrna.list[[5]] = CreateSeuratObject(counts = data.10x[[5]], min.cells=3, project=samples[5]);
scrna.list[[5]][["DataSet"]] = samples[5];
scrna.list[[6]] = CreateSeuratObject(counts = data.10x[[6]], min.cells=3, project=samples[6]);
scrna.list[[6]][["DataSet"]] = samples[6];
scrna.list[[7]] = CreateSeuratObject(counts = data.10x[[7]], min.cells=3, project=samples[7]);
scrna.list[[7]][["DataSet"]] = samples[7];
scrna.list[[8]] = CreateSeuratObject(counts = data.10x[[8]], min.cells=3, project=samples[8]);
scrna.list[[8]][["DataSet"]] = samples[8];
scrna.list[[9]] = CreateSeuratObject(counts = data.10x[[9]], min.cells=3, project=samples[9]);
scrna.list[[9]][["DataSet"]] = samples[9];
scrna.list[[10]] = CreateSeuratObject(counts = data.10x[[10]], min.cells=3, project=samples[10]);
scrna.list[[10]][["DataSet"]] = samples[10];
scrna.list[[11]] = CreateSeuratObject(counts = data.10x[[11]], min.cells=3, project=samples[11]);
scrna.list[[11]][["DataSet"]] = samples[11];
scrna.list[[12]] = CreateSeuratObject(counts = data.10x[[12]], min.cells=3, project=samples[12]);
scrna.list[[12]][["DataSet"]] = samples[12];
scrna.list[[13]] = CreateSeuratObject(counts = data.10x[[13]], min.cells=3, project=samples[13]);
scrna.list[[13]][["DataSet"]] = samples[13];
scrna.list[[14]] = CreateSeuratObject(counts = data.10x[[14]], min.cells=3, project=samples[14]);
scrna.list[[14]][["DataSet"]] = samples[14];
```

OR:
```{r eval = FALSE}
for (i in 1:length(data.10x)) {
    scrna.list[[i]] = CreateSeuratObject(counts = data.10x[[i]], min.cells=3, project=samples[i]);
...
# Could not 
```{r message=FALSE, warning = FALSE}
sc.hassan <- merge(scrna.list[[1]], y=c(scrna.list[[2]], scrna.list[[3]], scrna.list[[4]], scrna.list[[5]], scrna.list[[6]], scrna.list[[7]], scrna.list[[8]]), add.cell.ids = c("CRN00224913", "CRN00224914", "CRN00224915", "CRN00224916", "CRN00224917", "CRN00224918", "CRN00224919", "CRN00224920"), project="hassan2022");

sc.vanitha <- merge(scrna.list[[9]], y=c(scrna.list[[10]], scrna.list[[11]], scrna.list[[12]], scrna.list[[13]], scrna.list[[14]]), add.cell.ids = c("CRN00224921", "CRN00224922", "CRN00224923", "CRN00224924", "CRN00224925", "CRN00224926"), project="vanitha2022");
rm(scrna.list); 
...
Extract counts for reads exploration
```{r}
metadata <- as.data.frame(df[[]])
metadata$cells <- rownames(metadata)
metadata$sample <- "control1"
```

# Data exploration before QC

## Exploration of the number of genes
Boxplot of the number of genes per cell
```{r message=FALSE}
boxplot(metadata$nFeature_RNA,
        ylab = "Number of genes per cell",
        xlab = "Control 1",
        cex.lab=1.5)
ggsave(paste0(outdir, "genes.per.cell.boxplot.png"))
```

Histogram of genes per cell
```{r message=FALSE}
hist(metadata$nFeature_RNA, 
     xlab = "Number of genes per cell", 
     breaks = 50, 
     cex.lab=1.5,
     main = "The distribution of the number of genes detected per cell")
ggsave(paste0(outdir, "genes.per.cell.histogram.png"))
```

Mean number of genes across cells
```{r comment=NA}
mean.gene.number <- mean(metadata$nFeature_RNA)
print(paste("Mean number of genes is", mean.gene.number))
```

Median number of genes across cells
```{r comment=NA}
median.gene.number <- median(metadata$nFeature_RNA)
print(paste("Median number of genes is", median.gene.number))
```

## Exploration of the number of UMI
Boxplot of the number of UMI per cell
```{r message=FALSE}
boxplot(metadata$nCount_RNA,
        ylab = "Number of UMI per cell",
        xlab = "Control 1",
        cex.lab=1.5) 
ggsave(paste0(outdir, "UMI.per.cell.boxplot.png"))
```

Histogram of UMI per cell
```{r message=FALSE}
hist(metadata$nCount_RNA, 
     xlab = "Number of UMI per cell", 
     breaks = 50, 
     cex.lab=1.5,
     main = "The distribution of the number of UMI detected per cell")
ggsave(paste0(outdir, "UMI.per.cell.histogram.png"))
```

Mean number of UMI across cells
```{r comment=NA}
mean.UMI.number <- mean(metadata$nCount_RNA)
print(paste("Mean number of UMI is", mean.UMI.number))
```

Median number of UMI across cells
```{r comment=NA}
median.UMI.number <- median(metadata$nCount_RNA)
print(paste("Median number of UMI is", median.UMI.number))
```
...
```{r message = FALSE, warning = FALSE}
df <- CreateSeuratObject(counts = data.adjusted, project = "control1", min.cells = 3)
```
