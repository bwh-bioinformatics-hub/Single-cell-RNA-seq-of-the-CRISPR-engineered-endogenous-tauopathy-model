...
rbPattern = args[11]
mitoCutoff = args[12]
treatments = unlist(strsplit(args[13], ','))
...
message("Clustered_DotPlot_relabel function from developer")
...
message("Remove intermediate file")
rm(ds)
rm(filt.matrix)
rm(meta)
rm(raw.matrix)
...
# elbow plot
pdf(file = paste0(outdir, "seurat_multi/", "elbow.plot.pdf"), width = 8, height = 8)
...
p1
pdf(file=paste0(outdir, "seurat_multi/", "combined.umap.colorBySample.pdf"))
print(p1 + coord_fixed())
dev.off()

p2 <- DimPlot(scrna.combined, reduction = "umap", label = TRUE, repel = TRUE)
p2
pdf(paste0(outdir, "seurat_multi/", "combined.umap.colorByCluster.pdf"))
print(p2 + coord_fixed())
dev.off()

p3 <- DimPlot(scrna.combined, reduction = "umap", split.by = "orig.ident", ncol = 4)
p3
pdf(paste0(outdir, "seurat_multi/", "combined.umap.samples.pdf"), width = 12, height = 8)
print(p3 + coord_fixed())
dev.off()

p4 <- DimPlot(scrna.combined, reduction = "umap", group.by = "treatment")
p4
pdf(file=paste0(outdir, "seurat_multi/", "combined.umap.colorByTreatment.pdf"))
...
write.table(scrna.markers, paste0(outdir, "seurat_multi/", "FindAllMarkers.clusters.xls"), sep = "\t", row.names = F)
scrna.markers.wide <- reshape(scrna.markers, idvar = c("geneID", "geneSymbol"), timevar = "cluster", direction = "wide")
write.table(scrna.markers.wide, paste0(outdir, "seurat_multi/", "FindAllMarkers.clusters.wide.xls"), sep = "\t", row.names = F)

topN <- scrna.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
write.table(topN, paste0(outdir, "seurat_multi/", "FindAllMarkers.clusters.top10.xls"), sep = "\t", col.names = NA)
...
ggsave(paste0(outdir, "seurat_multi/", "top10markers.heatmap.integrated.geneID.pdf"), width = 8, height = 12)
...
ggsave(paste0(outdir, "seurat_multi/", "top10markers.heatmap.integrated.geneSymbol.pdf"), width = 8, height = 12)

DefaultAssay(scrna.combined) <- "RNA"
DoHeatmap(scrna.combined, features = topN$geneID, size = 2, draw.lines = T, angle = 45, hjust = 0.2) + theme(axis.text.y = element_text(size = 5)) + NoLegend()
ggsave(paste0(outdir, "seurat_multi/", "top10markers.heatmap.RNA.geneID.pdf"), width = 8, height = 12)

DoHeatmap(scrna.combined, features = topN$geneID, size = 2, draw.lines = T, angle = 45, hjust = 0.2) + theme(axis.text.y = element_text(size = 5)) + NoLegend() + scale_y_discrete(breaks=topN$geneID,
        labels=geneTable$geneSymbol[match(topN$geneID, geneTable$geneID)])
ggsave(paste0(outdir, "seurat_multi/", "top10markers.heatmap.RNA.geneSymbol.pdf"), width = 8, height = 12)


message("Step 6: Top 3 identified genes, feature plot, dotplot")
topN <- Extract_Top_Markers(scrna.markers, num_genes = 3, named_vector = FALSE, make_unique = TRUE, gene_column = "geneID")

# Feature plot
pdf(paste0(outdir, "seurat_multi/", "combined.top3markers.geneID.pdf"))
ggp = list()
for (marker in topN){
...
pdf(paste0(outdir, "seurat_multi/", "combined.top3markers.geneSymbol.pdf"))
ggp = list()
for (marker in topN){
...
# Dotplot
pdf(paste0(outdir, "seurat_multi/", "combined.dotplot.DEtop3.geneID.pdf"), width = 20, height = 10)
p1 <- DotPlot_scCustom(scrna.combined, features = topN, x_lab_rotate = TRUE, colors_use = "blue")
print(p1)
dev.off()

pdf(paste0(outdir, "seurat_multi/", "combined.dotplot.DEtop3.geneSymbol.pdf"), width = 20, height = 10)
...
# developer
pdf(paste0(outdir, "seurat_multi/", "combined.dotplot.DEtop3.clustered.geneSymbol.pdf"), width = 10, height = 15)
...
pdf(paste0(outdir, "seurat_multi/", "combined.dotplot.DEtop3.clustered.geneID.pdf"), width = 10, height = 15)
p1 <- Clustered_DotPlot_relabel(scrna.combined, features = topN, x_lab_rotate = F, plot_km_elbow = FALSE, new_row_labels = topN)
print(p1)
dev.off()


message("Customer markers, feature plot, dotplot")
# Feature plot
markers.ID = geneTable$geneID[match(markers, geneTable$geneSymbol)]

pdf(paste0(outdir, "seurat_multi/", "combined.markers.geneID.pdf"))
ggp = list()
...
pdf(paste0(outdir, "seurat_multi/", "combined.markers.geneSymbol.pdf"))
ggp = list()
for (marker in markers.ID){
    ggp[[marker]]=FeaturePlot(scrna.combined, features=marker) + ggtitle(geneTable$geneSymbol[match(marker, geneTable$geneID)])
    print(ggp[[marker]])
...
# Dotplot
pdf(paste0(outdir, "seurat_multi/", "combined.dotplot.geneID.pdf"), width = 30, height = 10)
p1 <- DotPlot_scCustom(scrna.combined, features = markers.ID, x_lab_rotate = TRUE, colors_use = "blue")
print(p1)
dev.off()

pdf(paste0(outdir, "seurat_multi/", "combined.dotplot.geneSymbol.pdf"), width = 30, height = 10)
...
# developer
pdf(paste0(outdir, "seurat_multi/", "combined.dotplot.clustered.geneSymbol.pdf"), width = 10, height = 15)
...
pdf(paste0(outdir, "seurat_multi/", "combined.dotplot.clustered.geneID.pdf"), width = 10, height = 15)
...
message("Step 7: Save the data")
topN <- Extract_Top_Markers(scrna.markers, num_genes = 3, named_vector = FALSE, make_unique = TRUE, gene_column = "geneID")
saveRDS(scrna.combined, paste0(outdir, "seurat_multi/", "scrna.combined.seurat.", projectName, ".rds"))
