...
seuratdir="/Volumes/BIOINFORMATICS/projects/hassan2022/output/04_seurat/"
...
samples = c("CRN00224913", "CRN00224914", "CRN00224915")
```
...
for (j in 1:length(data.10x)) {
    scrna.list[[samples[j]]] = CreateSeuratObject(counts = data.10x[[j]], min.cells=3, project=samples[j])
    scrna.list[[samples[j]]][["DataSet"]] = samples[j]
}
```
...
# Step 4: Add percent.mt and percent.rb
Add percent.mt and percent.rb to cell level metadata
```{r}
for (i in 1:length(samples)) {
  scrna.list[[samples[i]]][["percent.mt"]] <- PercentageFeatureSet(scrna.list[[samples[i]]], pattern = "^mt:") 
  scrna.list[[samples[i]]][["percent.rb"]] <- PercentageFeatureSet(scrna.list[[samples[i]]], pattern = "^Rp[LS]")
}
```

# Step 5: Merge Seurat objects into an object
```{r eval = FALSE}
scrna <- scrna.list[[1]]
y.list=c()
...
# Step 6: Feature plot before QC
```{r}
# feature plot
for (sample in samples){
  VlnPlot(scrna.list[[sample]], 
          features = c("nFeature_RNA", "nCount_RNA", "percent.mt", "percent.rb"),
          ncol = 4,
          pt.size = 0.1) & theme(plot.title = element_text(size=10))
  ggsave(paste0(seuratdir, sample, ".feature.plot.png"), width = 8, height = 6)
}

# nCount_RNA vs. percent.mt
for (sample in samples){
  FeatureScatter(scrna.list[[sample]], feature1 = "nCount_RNA", feature2 = "percent.mt")
  ggsave(paste0(seuratdir, sample, ".nCount.vs.percent.mt.png"), width = 8, height = 6)
}

# nCount_RNA vs. nFeature_RNA
for (sample in samples){
  FeatureScatter(scrna.list[[sample]], feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
  ggsave(paste0(seuratdir, sample, ".nFeature.vs.nCount.png"), width = 8, height = 6)
}
```

# Step 7: Run doublet detection scripts
Run scrublet scripts. The scrublet script is a python script, this python script is put inside of a bash script. The following command is used to run bash script in R. 
```{r}
system2(command = "bash",
        args = c("run_scrublet_multi.sh"))
```

# Step 8: Read in doublet scores
Read in doublet result from Scrublet analysis done in Pyhon
```{r}
doublet.list = list()
for (sample in samples){
...
# Step 9: QC, filter cells with nCount, nFeature, percent.mito
...
for (sample in samples){
  VlnPlot(scrna.list[[sample]], 
          ...
          ncol = 4,
          pt.size = 0.1) & theme(plot.title = element_text(size=10))
  ggsave(paste0(seuratdir, sample, ".feature.plot.QC.png"), width = 8, height = 6)
}

# nCount_RNA vs. percent.mt
for (sample in samples){
  FeatureScatter(scrna.list[[sample]], feature1 = "nCount_RNA", feature2 = "percent.mt")
  ggsave(paste0(seuratdir, sample, ".nCount.vs.percent.mt.QC.png"), width = 8, height = 6)
}

# nCount_RNA vs. nFeature_RNA
for (sample in samples){
  FeatureScatter(scrna.list[[sample]], feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
  ggsave(paste0(seuratdir, sample, ".nFeature.vs.nCount.QC.png"), width = 8, height = 6)
}
```
